...
function activate(context) {
    ...
    const insertLogCommand = vscode.commands.registerCommand("magicConsoleLogger.insertLog", insertLog);
    const deleteAllLogsCommand = vscode.commands.registerCommand("magicConsoleLogger.deleteAllLogs", deleteAllLogs);
    const commentAllLogsCommand = vscode.commands.registerCommand("magicConsoleLogger.commentAllLogs", commentAllLogs);
    const uncommentAllLogsCommand = vscode.commands.registerCommand("magicConsoleLogger.uncommentAllLogs", uncommentAllLogs);
    context.subscriptions.push(insertLogCommand, deleteAllLogsCommand, commentAllLogsCommand, uncommentAllLogsCommand);
}
...
function deleteAllLogs() {
    ...
    if (edits.length > 0) {
        ...
        vscode.window.showInformationMessage(`Deleted ${edits.length} magic log(s)`);
    }
    else {
...
function commentAllLogs() {
    ...
    if (edits.length > 0) {
        ...
        workspaceEdit.set(document.uri, edits);
        vscode.workspace.applyEdit(workspaceEdit);
        vscode.window.showInformationMessage(`Commented ${edits.length} magic log(s)`);
    }
    else {
...
function uncommentAllLogs() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('console.log("🪄')) {
            ...
            if (commentMatch) {
                const range = new vscode.Range(new vscode.Position(i, commentMatch[1].length), new vscode.Position(i, commentMatch[0].length));
                edits.push(vscode.TextEdit.delete(range));
            }
        }
    }
    if (edits.length > 0) {
        ...
        workspaceEdit.set(document.uri, edits);
        vscode.workspace.applyEdit(workspaceEdit);
        vscode.window.showInformationMessage(`Uncommented ${edits.length} magic log(s)`);
    }
    else {
        vscode.window.showInformationMessage("No commented magic logs found");
    }
}
function getFunctionName(editor, position) {
    try {
        const document = editor.document;
        const text = document.getText();
...
function getFunctionNameFallback(editor, position) {
    const document = editor.document;
    ...
    for (let i = currentLine; i >= 0; i--) {
        ...
        const arrowMatch = line.match(/(?:const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=.*=>/);
        if (arrowMatch) {
            return arrowMatch[1];
        }
...
function getIndentation(editor, lineNumber) {
